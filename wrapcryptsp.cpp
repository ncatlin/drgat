...
static void wrapCryptcreatehash(void *wrapcxt, OUT void **user_data)
{
	...
	dr_fprintf(thread->f, "ARG,%d,"ADDR_FMT","ADDR_FMT",E,0,%x@", 1, drwrap_get_func(wrapcxt), retaddr, algid);
}
...
static void wrapCrypthashdata(void *wrapcxt, OUT void **user_data)
{
	...
	dr_fprintf(thread->f, "ARG,%d,"ADDR_FMT","ADDR_FMT",E,1,%s@", 1, drwrap_get_func(wrapcxt), thread->lastBlock->appc, data);
}
