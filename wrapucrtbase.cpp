...
static void wrapWcscpy(void *wrapcxt, OUT void **user_data)
{
	...
	dr_fprintf(thread->f, "ARG,%d,"ADDR_FMT","ADDR_FMT",E,1,%s@", 1, drwrap_get_func(wrapcxt), thread->lastBlock->appc, thread->stringbuf);
}
...
static void wrapFgets(void *wrapcxt, OUT void **user_data)
{
	...
	dr_fprintf(thread->f, "ARG,%d,"ADDR_FMT","ADDR_FMT",E,0,%d@", 1, drwrap_get_func(wrapcxt), thread->lastBlock->appc, num);
